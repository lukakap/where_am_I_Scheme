...
(define (intersection-points c_list)
  (if (null? c_list) '()
    (append (apply append (map (lambda (tmp) (intersect (car c_list) tmp)) 
                            (cdr c_list))) 
                            (intersection-points (cdr c_list)))))

(define (distance-product point points)
  (apply * (map (lambda (tmp) (dist point tmp)) 
                        (remove point points))))
...
(define (sort-points c_list)
  (quicksort c_list 
            (lambda (f_elem s_elem) (< (car f_elem) (car s_elem)))))


(define (clumped-points p_list)
  (apply append (map cdr (prefix-of-list 
          (sort-points (rate-points p_list))
					(quotient (length p_list) 2)))))

(define (average list)
  (/ (apply + list) (length list)))

(define (average-point points)
  (let ((avg (list (average (map car points))
                      (average (map cadr points)))))
        (list (distance-product avg points) avg)))

(define (best-estimate c_list)
  (average-point (clumped-points (intersection-points c_list))))

(define (where-am-i d_list s_list) 
  (sort-points(map best-estimate (all-guesses d_list s_list))))
